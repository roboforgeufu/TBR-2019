...
"""    def catch(self, release=False):
    Pega/Solta o bloco.
        if release:
            print("Opening...")
            self.claw.run_until_stalled(const.CLAWSP_DN, Stop.HOLD, const.CLAW_DTY_LIM)
    ...
    def lower_arm(self, release=False):
        if release:
            #soltar o bloco
            self.arm.run_time(const.LOWARM_SP,const.TIME_ARM, Stop.HOLD)
            self.claw.run_until_stalled(const.CLAWSP_UP, Stop.HOLD, const.CLAW_DTY_LIM)
            ...
            self.claw.set_dc_settings(100, 0)
            #sobe novamente
            self.arm.run_until_stalled(const.RAISEARM_SP, Stop.HOLD, const.CLAW_DTY_LIM)
